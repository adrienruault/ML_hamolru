...
def target_func(inputs_min_max_deg, output):
    ...
    answer = alg.tuner_degree_lambda(y = y_bin, x = std_x, degree_min = degree_min, degree_max = degree_max,\
                                        lambda_min = -5, lambda_max = 0, nb_lambda = 30, k_fold = 4, seeds = seeds,\
                                        max_iters = 100000, gamma = 1e-5, cost = "ridge", tol = 1e-4,\
...
def parallel_tuner():
    ...
    for i in range(30):
        ...
        file_i.write("Best degree: " + str(result_i[0][0]) + "with index: " + str(result_i[0][1]))
        file_i.write("\nBest lambda: " + str(result_i[1][0]) + "with index: " + str(result_i[1][1]))
        file_i.write("\n\nmean_rmse_te:")
